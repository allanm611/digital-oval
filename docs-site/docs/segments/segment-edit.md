# Edit Segment

## Overview

The Edit Segment page allows you to modify an existing segment's conditions, description, and configuration.

**[Insert screenshot of edit segment form]**

---

## When You Can Edit a Segment

You can edit a segment at any time with proper permissions. There are no restrictions based on segment status or usage.

---

## Editable Fields

The following fields can be modified:

**Segment Name*** (required)
- Descriptive name for your segment

**Description** (optional)
- Additional details about the segment's purpose

**Category** (optional)
- Assign the segment to a catalog category for organization

**Segment Conditions**
- Modify the criteria that determines segment membership
- Add, edit, or remove conditions
- Adjust condition logic (AND/OR)

**Tags**
- Add or remove tags for organization
- Use tags for filtering and categorization

---

## Editing a Segment

1. Click the **Edit** button on the [Segment List](./segments-list) or [Segment Details](./view-segment-details) page
2. Modify the desired fields
3. Update conditions if needed
4. Click **Save Segment** to apply the changes
5. The segment will be updated immediately

---

## Next Steps

After editing a segment, you can:
- View the updated segment details on the [Segment Details](./view-segment-details) page
- Return to the [Segment List](./segments-list) to manage other segments
- Recompute the segment to refresh membership with the new criteria

---
