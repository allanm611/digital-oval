  ...
  useEffect(() => {
    const loadDoc = async () => {
      try {
        ...
        const title = path.split("/").pop() || "Feature";
    ...
    <div className="w-full flex flex-col min-h-screen">
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 flex flex-col">
          <div className={`flex-1 px-5 lg:px-8 py-6 ${tw.primaryBackground}`}>
            <div className="max-w-3xl">
              ...
              {error && (
                <div className={`bg-[${color.status.danger}]/10 border border-[${color.status.danger}] rounded-md p-6 mb-6`}>
                  <div className="flex items-start gap-3">
                    ...
                    />
                    <div>
                      <h2 className={`font-semibold text-[${color.status.danger}] mb-1`}>
                        Document Not Found
                      </h2>
                      ...
                      <a
                        href="/docs"
                        className={`inline-flex items-center gap-2 text-emerald-500 hover:opacity-70 transition-opacity text-sm font-medium`}
                      >
                        <ChevronLeft size={16} />
                ...
                <div>
                  <article className="prose prose-sm max-w-none">
                    <ReactMarkdown
                      components={{
                        h1: (props) => (
                          <h1 className={`text-3xl font-bold mb-6 ${tw.textPrimary}`} {...props} />
                        ),
                        h2: (props) => (
                          <h2 className={`text-2xl font-semibold mt-8 mb-4 ${tw.textPrimary}`} {...props} />
                        ),
                        h3: (props) => (
                          <h3 className={`text-xl font-semibold mt-6 mb-3 ${tw.textSecondary}`} {...props} />
                        ),
                        p: (props) => (
                          <p className={`mb-4 ${tw.textSecondary} leading-7`} {...props} />
                        ),
                        ul: (props) => (
                        ...
                        blockquote: (props) => (
                          <blockquote
                            className={`border-l-4 border-emerald-500 pl-4 my-4 ${tw.textSecondary} italic`}
                            {...props}
                          />
                        ),
                        table: (props) => (
