...
interface CreateSegmentTypeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onTypeCreated?: (typeId: number) => void;
}

export default function CreateSegmentTypeModal({
  isOpen,
  onClose,
  onTypeCreated,
}: CreateSegmentTypeModalProps) {
  ...
  const handleCreateType = async () => {
    ...
    try {
      setIsCreating(true);
      const response = await segmentTypeService.createSegmentType({
        ...
        <div className="p-4 sm:p-6">
          <div className="mb-4">
            ...
            <Input
              ...
              placeholder="e.g., Behavioral, Demographic, RFM..."
              required
            />
          </div>
          ...
          <div className="mb-6">
            ...
            <textarea
              ...
              rows={3}
              className={`w-full px-3 py-2 border border-gray-300 ${tw.rounded} text-sm focus:outline-none`}
              placeholder="Segment type description..."
            />
          </div>
