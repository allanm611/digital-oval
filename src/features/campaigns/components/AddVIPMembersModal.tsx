...
export default function AddVIPMembersModal({
  ...
  useEffect(() => {
    if (isOpen) {
      setPage(1);
      setSelectedVIPListId(vipLists.length > 0 ? String(vipLists[0].id) : "");
      loadCustomers();
    }
  }, [isOpen]);
  ...
  const handleConfirm = () => {
    if (onAdd && selectedMembers.length > 0) {
      ...
      <div className={`bg-white ${tw.rounded} w-full max-w-4xl max-h-[90vh] flex flex-col`}>
        ...
        <div className="flex items-center justify-between p-6 border-t border-gray-200 flex-shrink-0">
          <div className="text-sm text-black">
            {selectedMembers.length} total customer
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              className={`px-5 py-2 ${tw.rounded} text-sm font-medium transition-opacity ${
                selectedMembers.length === 0 || isLoading ? "cursor-not-allowed opacity-50" : ""
              }`}
              style={{
                backgroundColor:
