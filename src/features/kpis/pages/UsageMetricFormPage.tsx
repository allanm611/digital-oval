...
export default function UsageMetricFormPage() {
  ...
  const loadMetric = async () => {
    ...
    try {
      setLoading(true);
      const metric = await usageMetricService.getMetricById(Number(id));
      if (metric) {
        setFormData({
          name: metric.name,
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <p className="text-gray-500 font-medium mt-4">Loading usage metric...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <KPIForm
        ...
        onCancel={() => navigate("/dashboard/kpis/usage-metrics")}
