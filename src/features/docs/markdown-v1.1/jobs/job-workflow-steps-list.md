# Job Workflow Steps List

Job Workflow Steps List is the operational table for step definitions.

## Table Fields

- Step Name / Code
- Job ID
- Step Type
- Step Order
- On Failure Action
- Is Critical
- Is Parallel
- Retry Count
- Active State

## Filters

Search by name/code and filter by step type, job, or failure behavior.

![Job workflow steps list page](/img/v1.1/jobmanagement-images/jobworkflowstepslistpage.png)

## Actions

- Create
- View
- Edit
- Delete
- clone/validate/execute actions where enabled

![Job workflow steps bulk operations](/img/v1.1/jobmanagement-images/jobworkflowstepsbulkoperationsimage.png)

## Related Pages

- [Job Workflow Steps](/documentation/jobs/job-workflow-steps)
- [Create Job Workflow Step](/documentation/jobs/create-job-workflow-step)
- [View Job Workflow Step](/documentation/jobs/view-job-workflow-step)
- [Job Workflow Steps Analytics](/documentation/jobs/job-workflow-steps-analytics)
