...
Campaign types can be enabled or disabled by administrators in the Campaign Types configuration page. Only active types are available for selection when creating a new campaign. Deactivated types are hidden from the campaign creation flow until reactivated.
