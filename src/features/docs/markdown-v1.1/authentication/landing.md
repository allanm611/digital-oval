# Landing Page

## Overview

The Landing Page is the first page users see when visiting the Sentra CVM platform. It provides an overview of the platform's key features and modules, along with calls-to-action for logging in or requesting an account.

![Landing Page](/img/v1.1/auth-images/landingpage.png)


## Hero Section

The hero section displays:
- Platform branding and logo
- Platform name and tagline
- Benefits list highlighting key platform advantages
- Call-to-action buttons for [Login](/documentation/authentication/login) and [Request Account](/documentation/authentication/registration)


## Platform Features

The landing page showcases the platform's main capabilities and benefits, helping new users understand the value of the Sentra CVM solution.


## Modules Overview

### Campaign Management

Manage multi-step campaigns to reach customers with targeted messaging across multiple channels.

### Offer Management

Create and manage targeted offers to drive customer engagement and increase revenue.

### Segment Management

Build customer segments based on profile attributes, behaviors, and historical data to enable precise targeting.

### Analytics Reporting

Track campaign performance, audience engagement, and ROI with comprehensive analytics dashboards.


## Navigation

From the landing page, you can:
- **Login** - Access your existing account
- **Request Account** - Start the process of requesting a new account
- View detailed information about platform capabilities
