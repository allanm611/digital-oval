...
- Segment types can be enabled or disabled by administrators in the Segment Types configuration page.
- Only active (enabled) segment types are available for selection when creating or editing segments.
- If a segment type is deactivated, it will no longer appear as an option in segment type dropdowns or selection lists.

**Deactivated Segment Types**

- Deactivated segment types are hidden from all segment creation and editing flows until reactivated.
- To use a deactivated segment type again, activate it in the Segment Types configuration page.
- This ensures only valid, approved segment categories are available to users.
...
- Engagement level groupings (active, dormant, inactive)
...
- Income level or socioeconomic classification
...
- Site-level geographic metadata
