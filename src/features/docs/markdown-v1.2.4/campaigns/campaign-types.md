...
![Campaign Types](/img/v1.1/configuration/campaigntypes.png)
...
- Campaign types can be enabled or disabled by administrators in the Campaign Types configuration page.
- Only active (enabled) campaign types are available for selection when creating a new campaign (Step 2: Campaign Configuration).
- If a campaign type is deactivated, it will no longer appear as an option in the campaign type selector or in any campaign creation dropdowns.

**Deactivated Campaign Types**

- Deactivated campaign types are hidden from the campaign creation flow and cannot be selected until reactivated.
- To use a deactivated campaign type again, activate it in the Campaign Types configuration page.
- This ensures only valid, approved campaign workflows are available to users.
