...
export const resourceTypesConfig: TypeConfigurationPageConfig = {
  ...
  metadataFields: [
    ...
    },
    {
      ...
      required: true,
      options: [
        { value: "KB", label: "KB (Data)" },
        { value: "MB", label: "MB (Data)" },
        { value: "GB", label: "GB (Data)" },
        { value: "SECONDS", label: "Seconds (Voice)" },
        { value: "MINUTES", label: "Minutes (Voice)" },
        { value: "COUNT", label: "Count (SMS/Messaging)" },
        { value: "CURRENCY", label: "Currency (Monetary)" },
        { value: "PERCENTAGE", label: "Percentage (Discount)" },
        { value: "POINTS", label: "Points (Loyalty)" },
        { value: "SESSIONS", label: "Sessions (Streaming)" },
...
export function getTimezonesApiConfig(
  ...
  return {
    ...
    metadataFields: [
      ...
      {
        label: "Uses Daylight Saving Time (DST)",
        key: "uses_dst",
        type: "toggle",
        row: 4,
      },
      {
        ...
        condition: (values) => values.uses_dst === true || values.uses_dst === "true",
        row: 5,
      },
      {
      ...
      },
      {
        ...
        key: "sort_order",
        type: "number",
        placeholder: "Numeric order for dropdown sorting",
        row: 7,
      },
    ],
    createEditPath: "/dashboard/timezones",
  } as APIConfigurationPageConfig;
}
