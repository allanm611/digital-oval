...
export default function CreateRoutePage() {
  ...
  const loadChannels = async () => {
    try {
      ...
      if (allChannels && allChannels.length > 0) {
        setFormData((prev) => ({
          ...prev,
          channel: getChannelType(allChannels[0].code),
        }));
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.channel === "SMS") {
        await smsRouteService.createRoute({
          ...baseData,
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <HeadlessSelect
                ...
                onChange={(value) => {
                  const channelType = getChannelType(value);
                  setFormData({
                    ...formData,
                    channel: channelType,
                    gateway_config_id: 0,
                ...
                options={channels.map((channel) => ({
                  value: getChannelType(channel.code),
                  label: channel.name,
                }))}
        ...
        {/* Backup Route Section (SMS, PUSH, WHATSAPP, USSD only) */}
        {(formData.channel === "SMS" || formData.channel === "PUSH" || formData.channel === "WHATSAPP" || formData.channel === "USSD") && (
          <div className={`${tw.rounded} border border-gray-200 bg-white p-6 shadow-sm`}>
            <h2 className={`text-lg font-semibold ${tw.textPrimary} mb-4`}>
              Failover Settings
            </h2>
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  onChange={(checked) => setFormData({ ...formData, use_backup_on_failure: checked })}
                  disabled={saving}
                />
                <label htmlFor="use_backup" className={`text-sm font-medium text-gray-700`}>
          ...
          <div className={`${tw.rounded} border border-gray-200 bg-white p-6 shadow-sm`}>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(value) => setFormData({ ...formData, senderId: value })}
        ...
        <div className="flex gap-3 justify-end">
          <button
            ...
            disabled={saving}
            className={`border border-gray-300 text-gray-700 font-medium px-4 py-2 ${tw.rounded} transition-colors hover:bg-gray-50`}
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={saving}
            className={`text-white font-medium px-4 py-2 ${tw.rounded} transition-colors flex items-center justify-center gap-2 ${
              saving ? "opacity-50 cursor-not-allowed" : "hover:opacity-90"
            }`}
            style={{ backgroundColor: color.primary.action }}
          >
            {saving && <LoadingSpinner size={16} />}
            {saving ? "Creating..." : <><Save className="w-4 h-4" /> Create Route</>}
          </button>
        </div>
      </form>
    </div>
